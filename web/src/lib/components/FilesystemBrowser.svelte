...
<div class="space-y-3">
	...
	{#if error}
		<div
			class="p-3 bg-status-blocked/10 border border-status-blocked/30 rounded-lg text-sm text-status-blocked"
		>
			{error}
		</div>
	{/if}
			...
			{#each entries as entry (entry.path)}
				{#if entry.is_dir}
					...
					>
						<!-- Folder icon -->
						<svg
							class="w-4 h-4 flex-shrink-0 {entry.is_git_repo
								? 'text-primary-400'
								: 'text-text-muted'}"
							viewBox="0 0 24 24"
							fill="currentColor"
						...
						{#if entry.is_git_repo}
							<span
								class="flex-shrink-0 text-xs px-1.5 py-0.5 rounded bg-primary-600/20 text-primary-400 font-medium"
							>
								git
							</span>
						{/if}
						<!-- Chevron right -->
						<svg
							class="w-4 h-4 flex-shrink-0 text-text-muted ml-auto"
							viewBox="0 0 24 24"
							fill="currentColor"
						>
