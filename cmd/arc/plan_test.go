...
// The durable CLI is introduced in T6. Legacy wait must fail with upgrade
// guidance immediately instead of polling a removed global endpoint.
func TestPlanWaitRequiresDurableUpgrade(t *testing.T) {
	err := planWaitCmd.RunE(planWaitCmd, []string{"plan.legacy"})
