...
func TestSequencingConfirmationPrintsAfterCreatedLine(t *testing.T) {
	_, _, releaseID := setupSequencingTest(t)
	...
	seqIdx := strings.Index(out, "Sequenced after")
	require.NotEqual(t, -1, createdIdx, "expected Created: line in output: %q", out)
	require.NotEqual(t, -1, seqIdx, "expected Sequenced after line in output: %q", out)
	require.Less(t, createdIdx, seqIdx, "Created: line should print before Sequenced after line: %q", out)
}

func TestSequencingJSONOutputIsPureJSON(t *testing.T) {
	_, _, releaseID := setupSequencingTest(t)
