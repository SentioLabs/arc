...
func TestNormalizePath(t *testing.T) {
	...
	result := core.NormalizePath(tmpDir)
	if result == "" {
...
func TestNormalizePathPair(t *testing.T) {
	...
	realResolved := core.NormalizePath(realDir)
...
func TestNormalizePathPair_NoSymlink(t *testing.T) {
	...
	dir := t.TempDir()
	resolved := core.NormalizePath(dir) // canonical form

	absPath, resolvedPath := core.NormalizePathPair(resolved)
	if absPath != resolvedPath {
