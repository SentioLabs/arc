...
Deep dive into arc's four dependency types: blocks, related, parent-child, and discovered-from.

## Contents

- [Overview](#overview) - Four types at a glance, which affect arc ready?
...
Arc supports four dependency types that serve different purposes in organizing and tracking work:

| Type | Purpose | Affects `arc ready`? | Common Use |
|------|---------|---------------------|------------|
...
**Effect**: Issue B disappears from `arc ready` until issue A is closed.
...
Effect: api-endpoint-2 won't show in arc ready until db-schema-1 closed
```
...
Effect: arc ready shows only migrate-1; closing it reveals migrate-2, etc.
```
...
```bash
arc dep add prerequisite-issue blocked-issue
# or explicitly:
arc dep add prerequisite-issue blocked-issue --type blocks
```
...
arc ready shows only current step.
```
...
```
1. Close db-schema-1
2. Arc automatically updates: api-endpoint-2 is now ready
3. arc ready shows api-endpoint-2
4. No manual unblocking needed
```

This is why `blocks` is powerful - arc maintains ready state automatically.
...
**Effect**: No impact on `arc ready`. Pure informational link.
...
Effect: Both show in arc ready; choosing one doesn't block the other
```

### Creating related Dependencies

```bash
arc dep add issue-1 issue-2 --type related
```
...
**Effect**: No impact on `arc ready`. Creates hierarchical structure.
...
Effect: Hierarchical structure; all show in arc ready (unless blocked)
```
...
```bash
arc dep add child-task-id parent-epic-id --type parent-child
```
...
All children show in arc ready immediately.
...
arc ready shows only first child.
...
**Effect**: No impact on `arc ready`. Tracks origin and provides context.
...
```bash
arc dep add original-work-id discovered-issue-id --type discovered-from
```
...
**Problem**: No parallel work possible; `arc ready` shows only one issue.
...
**Wrong**:
```bash
arc dep add api-endpoint database-schema
...
**Right**:
```bash
arc dep add database-schema api-endpoint
