...
| File | Change |
|------|--------|
| `internal/workspace/naming.go` | Increase basename truncation from 5 → 10 in `GeneratePrefix()` and `GeneratePrefixFromName()`. Add `GeneratePrefixWithCustomName(dirPath, customName)` that uses the custom name as basename with the path-derived hash. |
...
| `internal/workspace/naming_test.go` | Update truncation and max-length expectations. Add tests for custom name function. |
