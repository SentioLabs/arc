...
**Architecture:** The change flows through three layers: CLI flag parsing (`cmd/arc/init.go`), prefix generation logic (`internal/workspace/naming.go`), and validation (`internal/types/types.go`). A new `GeneratePrefixWithCustomName` function handles the custom basename case while reusing the existing path-based hash suffix for uniqueness.
...
Run: `go test ./internal/types/ -run TestWorkspaceValidation -v`
...
**Files:**
- Modify: `internal/workspace/naming.go:126-130` (`GeneratePrefix`)
- Modify: `internal/workspace/naming.go:153-156` (`GeneratePrefixFromName`)
- Modify: `internal/workspace/naming_test.go`

**Step 1: Update test expectations for new truncation length**

In `internal/workspace/naming_test.go`, update `TestGeneratePrefixTruncation`:
...
Run: `go test ./internal/workspace/ -run "TestGeneratePrefix" -v`
Expected: FAIL — truncation is still at 5

**Step 3: Update truncation in GeneratePrefix and GeneratePrefixFromName**

In `internal/workspace/naming.go`, in `GeneratePrefix()` change:
...
```bash
git add internal/workspace/naming.go internal/workspace/naming_test.go
...
**Files:**
- Modify: `internal/workspace/naming.go` (add new function after `GeneratePrefix`)
- Modify: `internal/workspace/naming_test.go` (add new test)

**Step 1: Write the failing test**

Add to `internal/workspace/naming_test.go`:
...
Run: `go test ./internal/workspace/ -run TestGeneratePrefixWithCustomName -v`
Expected: FAIL — function does not exist

**Step 3: Implement GeneratePrefixWithCustomName**

Add to `internal/workspace/naming.go` after `GeneratePrefix`:
