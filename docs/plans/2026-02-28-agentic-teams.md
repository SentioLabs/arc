...
func TestGetTeamContext(t *testing.T) {
    ...
    req := httptest.NewRequest(http.MethodGet,
        fmt.Sprintf("/api/v1/workspaces/%s/team-context?epic_id=%s", wsID, epicID), nil)
...
GET /api/v1/workspaces/:ws/team-context?epic_id=...
...
```typescript
export async function getTeamContext(workspaceId: string, epicId?: string): Promise<TeamContext> {
    const params = epicId ? `?epic_id=${epicId}` : '';
    const response = await fetch(`${baseUrl}/api/v1/workspaces/${workspaceId}/team-context${params}`);
...
Add the `GET /workspaces/{ws}/team-context` endpoint with:
